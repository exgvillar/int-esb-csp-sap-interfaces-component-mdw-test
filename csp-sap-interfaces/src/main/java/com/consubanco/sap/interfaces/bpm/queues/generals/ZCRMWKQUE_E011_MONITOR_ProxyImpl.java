...
public class ZCRMWKQUE_E011_MONITOR_ProxyImpl implements IStandardSAPProxy {

	private static Log log = LogFactory.getLog( ZCRMWKQUE_E011_MONITOR_ProxyImpl.class );
	
	public ZCRMWKQUE_E011_MONITOR_ProxyImpl() {
		super();		
	}
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction( JCoDestination destination, ArrayList<ProxyInputParameter> inputParamtersList) throws Exception {
		...
		try {
			...
			ft = destination.getRepository().getFunctionTemplate( "ZCRMWKQUE_E011_MONITOR" );
			if(ft == null)
				throw new RuntimeException("ZCRMWKQUE_E011_MONITOR not found in SAP.");
...
//			log.info( function.getImportParameterList().toString() );
			...
			for (int i = 0; i < data.getNumRows(); i++, data.nextRow() ) {
				...
				ZCRMWKQUE_E011_MONITOR_OutputData tableData = new ZCRMWKQUE_E011_MONITOR_OutputData();
				...
				tableData.setNotificationRole( data.getString("ZNOTIF_ROL") );
				tableData.setRecapture( data.getString("ZRECAPTURA").equals("X") ? true : false );
				tableData.setExecuted( data.getString("ZEXECUTED").equals("X") ? true : false );
				...
				tableData.setConvenioId( data.getString("ZCONVENIO") );
				tableData.setConvenioDesc( data.getString("ZDESC_CONV") );
				tableData.setProductId( data.getString("ZPRODUCTO") );
				tableData.setProductDesc( data.getString("ZDESC_PROD") );
				tableData.setRequestAmount( data.getDouble("ZMONTO") );
				tableData.setWaitTime( data.getLong("ZTIEMPO_ESPERA") );
				tableData.setRecaptureNumber( data.getInt("ZNUM_RECAPTURA") );
				tableData.setSourceChannelId( data.getString("ID_CANAL_ORI") ); 
				tableData.setSourceChannelDesc( data.getString("DESC_CANAL_ORI") ); 
				
				returnList.add( tableData );	
			}	
	
		} catch (AbapException e) {
			log.error(" executing function " + "ZCRMWKQUE_E011_MONITOR :: " + function.getExportParameterList().getString("E_MESSAGE") , e);
			throw e;
		} finally {
