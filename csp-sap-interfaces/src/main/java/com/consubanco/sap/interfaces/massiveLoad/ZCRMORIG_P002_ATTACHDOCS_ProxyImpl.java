...
public class ZCRMORIG_P002_ATTACHDOCS_ProxyImpl {

	private static Log log = LogFactory.getLog( ZCRMORIG_P002_ATTACHDOCS_ProxyImpl.class );
	
	public ZCRMORIG_P002_ATTACHDOCS_ProxyImpl() {
		super();		
	}
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction( JCoDestination destination, ArrayList<ProxyInputParameter> inputParamtersList, File file) throws Exception {
		...
		try {
			...
			ft = destination.getRepository().getFunctionTemplate("ZCRMORIG_P002_ATTACHDOCS");
			if(ft == null)
	            throw new RuntimeException("ZCRMORIG_P002_ATTACHDOCS not found in SAP.");
			...
			ZCRMORIG_P002_ATTACHDOCS_OutputData output = new ZCRMORIG_P002_ATTACHDOCS_OutputData();
			output.setUrl( function.getExportParameterList().getString("E_DOCUMENT_URL") );
			...
			log.error(" executing function " + "ZCRMORIG_P002_ATTACHDOCS" , e);
			throw e;
		} finally {
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction( JCoDestination destination, ArrayList<ProxyInputParameter> inputParamtersList, String base64Content ) throws Exception {
		...
		try {
			...
			function.getImportParameterList().setValue("I_FILECONTENT", decodedBytes );
