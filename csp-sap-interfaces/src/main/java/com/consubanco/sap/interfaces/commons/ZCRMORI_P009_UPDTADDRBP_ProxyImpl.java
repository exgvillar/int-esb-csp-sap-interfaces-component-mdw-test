...
public class ZCRMORI_P009_UPDTADDRBP_ProxyImpl  {

	private static Log log = LogFactory.getLog( ZCRMORI_P009_UPDTADDRBP_ProxyImpl.class );
	
	
	public ZCRMORI_P009_UPDTADDRBP_ProxyImpl() {
		super();		
	}
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction( JCoDestination destination, ProxyInputParameterWithTable inputParamters ) throws Exception {
		...
		try {
			...
			ft = destination.getRepository().getFunctionTemplate( "ZCRMORI_P009_UPDTADDRBP" );
			if(ft == null)
				throw new RuntimeException("ZCRMORI_P009_UPDTADDRBP not found in SAP.");
			...
			log.info("Input >>> " + tableInput);

			function.execute(destination);
			
			JCoTable data = function.getExportParameterList().getTable("E_ADDRESS");
			boolean success = "X".equalsIgnoreCase(function.getExportParameterList().getString("E_SUCCESS"));
			
			log.info("E_ADDRESS >>> " +  data.toString() );
			log.info("E_SUCCESS >>> " +  function.getExportParameterList().getString("E_SUCCESS"));
			...
			log.error(" executing function " + "ZCRMORI_P009_UPDTADDRBP" , e);
			throw e;
		} finally {
