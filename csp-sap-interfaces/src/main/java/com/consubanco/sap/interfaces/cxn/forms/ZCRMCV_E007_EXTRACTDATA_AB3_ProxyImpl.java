...
public class ZCRMCV_E007_EXTRACTDATA_AB3_ProxyImpl {

	private static Log log = LogFactory.getLog( ZCRMCV_E007_EXTRACTDATA_AB3_ProxyImpl.class );
	
	public ZCRMCV_E007_EXTRACTDATA_AB3_ProxyImpl() {
		super();		
	}
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction(JCoDestination destination, ProxyInputParameterWithTable inputParamters) throws Exception {
		...
		try {
			...
			ft = destination.getRepository().getFunctionTemplate( "ZCRMCV_E007_EXTRACTDATA_AB3" );
			if(ft == null)
				throw new RuntimeException("ZCRMCV_E007_EXTRACTDATA_AB2 not found in SAP.");
			
			function = ft.getFunction();
	
			Iterator<ProxyInputParameter> iter = inputParamters.getGeneralParam().iterator();
			while( iter.hasNext() ) {
			...
			// record - table params
			JCoTable tableInput = function.getImportParameterList().getTable("IT_DOCUMENTOS");
			...
			log.info("Function.IT_DOCUMENTOS: " + tableInput.toString());
			...
			log.error(" executing function " + "ZCRMCV_E007_EXTRACTDATA_AB2" , e);
			throw e;
		} finally {
