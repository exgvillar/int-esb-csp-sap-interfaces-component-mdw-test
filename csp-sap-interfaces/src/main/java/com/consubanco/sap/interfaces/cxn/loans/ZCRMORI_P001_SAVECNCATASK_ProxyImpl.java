...
public class ZCRMORI_P001_SAVECNCATASK_ProxyImpl {
	
	private static Log log = LogFactory.getLog(ZCRMORI_P001_SAVECNCATASK_ProxyImpl.class);
	private final String rfc = "ZCRMORI_P001_SAVECNCATASK";

	public List<IProxyOutputData> executeRemoteFunction(JCoDestination destination,
			ZCRMORI_P001_SAVECNCATASK_Model inputParamsModel) throws Exception {
		
		ZCRMORI_P001_SAVECNCATASK_OutputData outputData = new ZCRMORI_P001_SAVECNCATASK_OutputData();
		...
		try {
			...
			if( inputParamsModel != null ) {
				...
				function.getImportParameterList().setValue("I_SOLICITUD", inputParamsModel.getSolicitudId());
