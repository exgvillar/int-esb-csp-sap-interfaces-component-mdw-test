...
public class ZCRMORI_P002_UPDSTSREQLETTER_ProxyImpl {
	
	private static Log log = LogFactory.getLog(ZCRMORI_P002_UPDSTSREQLETTER_ProxyImpl.class);
	private final String rfc = "ZCRMORI_P002_UPDSTSREQLETTER";
	
	public List<IProxyOutputData> executeRemoteFunction(JCoDestination destination,
			ZCRMORI_P002_UPDSTSREQLETTER_Model inputParamsModel) throws Exception {
		
		ZCRMORI_P002_UPDSTSREQLETTER_OutputData outputData = new ZCRMORI_P002_UPDSTSREQLETTER_OutputData();
		...
		try {
			...
			if( inputParamsModel != null ) {
				...
				function.getImportParameterList().setValue("I_STATUS", inputParamsModel.getStatus().getKey());
			}	
