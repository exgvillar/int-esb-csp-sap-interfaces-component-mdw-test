...
public class ZCRMORI_P003_SAVEREQLETTSIPRE_ProxyImpl {

	private static Log log = LogFactory.getLog(ZCRMORI_P003_SAVEREQLETTSIPRE_ProxyImpl.class);
	private final String rfc = "ZCRMORI_P003_SAVEREQLETTSIPRE";
	
	public List<IProxyOutputData> executeRemoteFunction(JCoDestination destination,
			ZCRMORI_P003_SAVEREQLETTSIPRE_Model inputParamsModel) throws Exception {
		
		ZCRMORI_P003_SAVEREQLETTSIPRE_OutputData outputData = new ZCRMORI_P003_SAVEREQLETTSIPRE_OutputData();
		...
		try {
			...
			if( inputParamsModel != null ) {
				...
				if(inputParamsModel.getAccountStatus() != null) {
					function.getImportParameterList().setValue("I_ACCOUNTSTATUS", inputParamsModel.getAccountStatus().getKey());
				}
