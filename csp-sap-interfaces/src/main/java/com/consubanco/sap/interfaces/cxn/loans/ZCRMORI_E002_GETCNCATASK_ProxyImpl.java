...
public class ZCRMORI_E002_GETCNCATASK_ProxyImpl {
	
	private static Log log = LogFactory.getLog(ZCRMORI_E002_GETCNCATASK_ProxyImpl.class);
	private final String rfc = "ZCRMORI_E002_GETCNCATASK";

	public List<IProxyOutputData> executeRemoteFunction(JCoDestination destination,
			ZCRMORI_E002_GETCNCATASK_Model inputParamsModel) throws Exception {
		
		ZCRMORI_E002_GETCNCATASK_OutputData outputData = new ZCRMORI_E002_GETCNCATASK_OutputData();
		...
		try {
			...
			if( inputParamsModel != null ) {
				...
				function.getImportParameterList().setValue("I_FINDFILES", inputParamsModel.isFindFiles() ? "X" : "");
			}			
			...
			GENERIC_CATALOG_Model statusTask = new GENERIC_CATALOG_Model();
			statusTask.setKey(outTask.getString("E_STATUS"));
			statusTask.setDescription(outTask.getString("E_STATUS_DESC"));
			outputData.setStatus(statusTask);	
			
			JCoTable outTableLoan = outTask.getTable("E_LOANS");	
			log.info(outTableLoan.toString());
			
			log.info("taotal " + outTableLoan.getNumRows());
