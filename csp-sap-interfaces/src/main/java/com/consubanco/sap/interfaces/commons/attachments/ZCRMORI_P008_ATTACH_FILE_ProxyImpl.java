...
public class ZCRMORI_P008_ATTACH_FILE_ProxyImpl {

	private static Log log = LogFactory.getLog( ZCRMORI_P008_ATTACH_FILE_ProxyImpl.class );
	
	public ZCRMORI_P008_ATTACH_FILE_ProxyImpl() {
		super();		
	}
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction( JCoDestination destination, ArrayList<ProxyInputParameter> inputParamtersList, File file) throws Exception {
		...
		try {
			...
			ft = destination.getRepository().getFunctionTemplate("ZCRMORI_P008_ATTACH_FILE");
			if(ft == null)
	            throw new RuntimeException("ZCRMORI_P008_ATTACH_FILE not found in SAP.");
			...
			function.getImportParameterList().setValue("LV_BIN", out.toByteArray() );
			
			function.execute(destination);
			
			ZCRMORI_P008_ATTACH_FILE_OutputData output = new ZCRMORI_P008_ATTACH_FILE_OutputData();
			output.setUrl( function.getExportParameterList().getString("LV_DOCUMENT_URL") );
			...
			log.error(" executing function " + "ZCRMORI_P008_ATTACH_FILE" , e);
			throw e;
		} finally {
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction( JCoDestination destination, ArrayList<ProxyInputParameter> inputParamtersList, String base64Content ) throws Exception {
		...
		try {
			...
			function.getImportParameterList().setValue("LV_BIN", decodedBytes );
