...
public class Z_CRMRFC_ATTACHPROD_ProxyImpl {

	private static Log log = LogFactory.getLog( Z_CRMRFC_ATTACHPROD_ProxyImpl.class );
	
	public Z_CRMRFC_ATTACHPROD_ProxyImpl() {
		super();		
	}
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction( JCoDestination destination, ArrayList<ProxyInputParameter> inputParamtersList, File file) throws Exception {
		...
		try {
			...
			ft = destination.getRepository().getFunctionTemplate("Z_CRMRFC_ATTACHPROD");
			if(ft == null)
	            throw new RuntimeException("Z_CRMRFC_ATTACHPROD not found in SAP.");
			...
			function.getImportParameterList().setValue("DOCUMENTBINARYSTREAM", out.toByteArray() );
			
			function.execute(destination);
			
			
		} catch (AbapException e) {
			log.error(" executing function " + "Z_CRMRFC_ATTACHPROD" , e);
			throw e;
		} finally {
	...
	public ArrayList<IProxyOutputData> executeRemoteFunction( JCoDestination destination, ArrayList<ProxyInputParameter> inputParamtersList, String base64Content ) throws Exception {
		...
		try {
			...
			function.getImportParameterList().setValue("DOCUMENTBINARYSTREAM", decodedBytes );
